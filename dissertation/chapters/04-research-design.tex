...
\textcite{lieber2024} benchmarked LLMs' ability to produce valid JSON across six tasks and found an average success rate of 82.55\%, with rates ranging from 0\% to 100\% depending on the task and model. The content plan schema in this project is moderately complex: nested arrays of slide specifications, each containing typed content blocks with varying structures for charts, tables, and text. GPT-4o sometimes wraps the response in an unexpected outer object or omits optional fields, though all 12 evaluation runs produced valid output.

These findings shaped the validation strategy. Rather than relying on the model to always produce valid output, the system validates every response against a Zod schema and falls back to a generic plan on failure. This treats structured output as a best-effort capability that needs a safety net, not a guarantee.
