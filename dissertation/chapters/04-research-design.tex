...
The architecture separates concerns into independent modules (SE2, SE9). The Template Analyser extracts layout patterns without generating content. The Content Planner uses an LLM to structure the narrative without touching PowerPoint files. The Slide Builder assembles files from specifications without making content decisions. This separation has two practical benefits. First, each component can be tested in isolation: the template analyser can be tested against real .pptx files without needing an LLM call, and the slide builder can be tested against fixed content plans without waiting for GPT-4o. Second, components can be reused independently. The template analyser, for example, could serve as a standalone tool for cataloguing corporate templates without any AI generation, which is the first recommendation in Section~\ref{sec:evaluation}.
