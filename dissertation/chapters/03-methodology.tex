...
\hline
\textbf{ID} & \textbf{Risk} & \textbf{L} & \textbf{I} & \textbf{Backup Plan} & \textbf{Outcome} \\
\hline
R1 & API rate limits & M & M & Caching, staggered calls & Did not materialise; \\
 & & & & & 24h cache sufficient \\
\hline
R2 & LLM output & H & H & Schemas, validation, & Materialised: 22\% invalid \\
 & inconsistency & & & HITL review & on first attempt; Zod \\
 & & & & & retries raised to 96\% \\
\hline
R3 & Template analysis & M & H & Scope to common layouts, & Materialised: custom \\
 & fails on complex layouts & & & graceful fallback & placeholders (30\%); \\
 & & & & & heuristics raised to 93\% \\
\hline
R4 & External API & L & H & Abstract API layer, & Did not materialise \\
 & deprecation & & & fallback sources & \\
\hline
R5 & Scope creep & M & M & MoSCoW prioritisation, & Materialised: add-in added; \\
 & & & & regular scope reviews & RAG dropped via MoSCoW \\
\hline
R6 & Technical complexity & M & H & Iterative delivery, & Partially: Python-to-TS \\
 & exceeds time & & & defer stretch goals & migration cost 4 days; \\
 & & & & & recovered via faster builds \\
\hline
R7 & Data accuracy & M & H & Manual verification, & Did not materialise; API \\
 & issues & & & source attribution & passthrough approach \\
 & & & & & eliminated LLM fabrication \\
\hline
R8 & Compliance & L & H & Mandatory human review, & Did not materialise \\
 & violations & & & no PII processing & \\
\hline
\end{tabular}
\caption{Risk Assessment with Outcomes. L = Likelihood, I = Impact, M = Medium, H = High.}
\label{tab:risks}
\end{table}
