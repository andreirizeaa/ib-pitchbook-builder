...
All dependencies are pinned via \texttt{package-lock.json}, and the Node.js version (22.x) is specified in \texttt{.nvmrc}. LLM temperature is set to zero for evaluation runs. Each run is tied to a specific Git commit, and test templates are committed under \texttt{fixtures/}. The README documents all required environment variables and setup steps. GPT-4o does not guarantee identical outputs across API calls even at temperature zero \parencite{runeson2012}, so the Zod validation layer ensures structural consistency regardless of minor output variations.
