...
Git with simplified GitFlow (SE12) handles version control: \texttt{main} is always deployable, \texttt{develop} is the integration branch, and feature branches isolate work in progress. GitHub Actions CI runs lint, type checks, and tests on every push, enforcing 80\% coverage for core service modules \parencite{fowler2006} (SE6). ESLint \parencite{eslint2024} catches code style issues and common errors before tests run. Architecture Decision Records \parencite{tyree2005} document technology choices so future developers can understand why specific trade-offs were made.
