...
Neither pipeline was in the original design. Both emerged from usability problems: without visual previews, users had to open the file in PowerPoint to see what was generated, which added friction. Adding preview images turned the web app into a useful management interface even after the add-in became the primary generation tool.
...
The \texttt{insertSlidesFromBase64()} API pushes the generated .pptx directly into the active presentation. Both the add-in and the web app connect to the same backend and database, so a deck generated in either interface appears in both. The web app handles browsing, preview, and history. The add-in handles generation and editing inside PowerPoint (K5).
...
\begin{table}[H]
\centering
\begin{tabular}{|l|p{3.8cm}|l|p{5.5cm}|}
\hline
